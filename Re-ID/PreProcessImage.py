...
class PreProcessIm(object):
  ...
  def pre_process_im(self, im):
    ...
    if ((self.hsv_jitter_prob > 0)
      ...
      im = self.hsv_jitter(im, self.hsv_jitter_range[0], self.hsv_jitter_range[1], self.hsv_jitter_range[2]) 
...
#      print kernel_size
    ...
    if self.image_save and save_img:
      ...
      save_img = ((im * np.array(self.im_std).astype(float)) + np.array(self.im_mean))*255
      print (save_img.shape)
      cv2.imwrite(image_path, cv2.cvtColor(save_img.astype(np.uint8), cv2.COLOR_RGB2BGR))
