...
class ResNet(nn.Module):
  ...
  def __init__(self, block, layers, last_conv_stride=2, last_conv_dilation=1, intermediate = False):
  ...
  def forward(self, x, from_layer = ''):
    #print 'input', x.size()

    if from_layer != '':
        if from_layer == 'layer1':
            x = self.layer2(x)
            x = self.layer3(x)
            x = self.layer4(x)
        elif from_layer == 'layer2':
            x = self.layer3(x)
            x = self.layer4(x)
        elif from_layer == 'layer3':
            x = self.layer4(x)
        return x
    ...
    #print 'conv1, maxpooling', x.size()
    output_layer1 = self.layer1(x)
    #print 'layer1 out', output_layer1.size()
    output_layer2 = self.layer2(output_layer1)
    #print 'layer2 out', output_layer2.size()
    output_layer3 = self.layer3(output_layer2)
    #print 'layer3 out', output_layer3.size()
    x = self.layer4(output_layer3)
    #print 'layer4 out', x.size()
    if self.intermediate:
      return output_layer1, output_layer2, output_layer3 ,x
    return x
