...
def load_ckpt(modules_optims, ckpt_file, load_to_cpu=True, verbose=True):
  ...
  if 'state_dicts' in ckpt:
    ...
    #load_state_dict(model, ckpt['state_dicts'][0])
  else:
    state_dict = ckpt['state_dict']
    #load_state_dict(model, ckpt['state_dict'])
  sibling = False
  for param_name in model.state_dict().keys():
    if 'base1' in param_name:
      sibling = True
      for param_name2 in state_dict.keys():
          if 'base1' in param_name2:
              sibling = False
      break
  if sibling:
    new_state_dict = {}
    for key in state_dict:
      #print (key)
      if 'base' in key:
        new_key = key.replace('base', 'base0')
        new_state_dict[new_key] = state_dict[key]
        new_key = key.replace('base', 'base1')
        new_state_dict[new_key] = state_dict[key]
        #print (new_state_dict.keys())
      if 'conv_list.0' in key:
        new_key = key.replace('local_conv_list.0', 'conv0')
        new_state_dict[new_key] = state_dict[key]
        new_key = key.replace('local_conv_list.0', 'conv1')
        new_state_dict[new_key] = state_dict[key]
      if 'fc_list.0' in key:
        new_key = key.replace('fc_list.0', 'fc')
        new_state_dict[new_key] = state_dict[key]
  ...
  if double:
    ...
    for target_key in target_dict:
      if target_key not in state_dict:
        src_key = target_key.replace('_double', '')
        src_key = src_key.replace('_single', '')
        if src_key in state_dict:
          new_state_dict[target_key] = state_dict[src_key]
      else:
        new_state_dict[target_key] = state_dict[target_key]
    state_dict = new_state_dict
...
#    load_state_dict(m, sd)
  if verbose and 'state_dicts' in ckpt:
